...
class DlProtectCom(SimpleCrypter):
    ...
    def conf(self):
        useragent = self.pyload.api.getConfigValue(
            "UserAgentSwitcher", "useragent", "plugin")
    ...
    def get_links(self):
        ...
        if not re.match(r'https?://(?:www\.)?dl-protect\.com/.+',
                        self.req.http.lastEffectiveURL):
        ...
        if "Please click on continue to see the links" in self.data:
            ...
            b64time = "_" + \
                base64.urlsafe_b64encode(str(mstime)).replace("=", "%3D")
        ...
        return re.findall(
            r'<a href="(?P<id>[^/].+?)" target="_blank">(?P=id)</a>', self.data)
