...
class FilesMailRu(Hoster):
    ...
    def get_file_url(self):
        ...
        """
        return re.search(self.url_pattern, self.data).group(0).split(
            '<a href="')[1].split('" onclick="return Act')[0]

    def get_file_name(self):
        """
        Gives you the Name for each file. Also extracted from the HTML-Page
        """
        return re.search(self.url_pattern, self.data).group(
            0).split(', event)">')[1].split('</a>')[0]
    ...
    def my_post_process(self):
        ...
        if self.scan_download({'html': "<meta name="},
                              read_size=50000) == "html":
